...
def gecmis_yukle():
    """Alış geçmişi sayfasını yükle"""
    try:
        if os.path.exists("market_fisi_urunler_2.xlsx"):
            xl = pd.ExcelFile("market_fisi_urunler_2.xlsx")
            if 'ALIŞ GEÇMİŞİ' in xl.sheet_names:
                df = pd.read_excel("market_fisi_urunler_2.xlsx", sheet_name='ALIŞ GEÇMİŞİ')
                # Encoding düzelt
                yeni_kolonlar = {}
...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .zam-badge { padding: 2px 10px; border-radius: 20px; font-size: 0.78rem; font-weight: 700; }
  .zam-up { background: #e74c3c22; color: #e74c3c; }
  .zam-down { background: #2ecc7122; color: #2ecc71; }
  ...
  .filtre-row input[type=date] { padding: 10px 14px; border-radius: 10px; border: none; background: #1a1a2e; color: #fff; font-size: 0.95rem; }
  .filtre-row select { padding: 10px 14px; border-radius: 10px; border: none; background: #1a1a2e; color: #fff; font-size: 0.95rem; }
  .card-row { display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 14px; margin-bottom: 20px; }
  ...
  /* GRAFİK */
  .grafik-wrap { background: #16213e; border-radius: 12px; padding: 20px; margin-bottom: 20px; }
  .grafik-wrap h3 { color: #eaeaea; margin-bottom: 14px; font-size: 1rem; }
  
  /* ZAM LİSTESİ */
  .zam-wrap { background: #16213e; border-radius: 12px; padding: 20px; margin-bottom: 20px; }
  .zam-wrap h3 { color: #eaeaea; margin-bottom: 14px; font-size: 1rem; }
...
<div class="sayfa active" id="sayfa-arama">
  <div class="search-row">
    ...
    <button class="btn btn-red" onclick="yenile()">🔄</button>
...
<div class="sayfa" id="sayfa-gecmis">
  <div class="filtre-row">
    <input type="date" id="tarih-bas" title="Başlangıç tarihi">
    <input type="date" id="tarih-bit" title="Bitiş tarihi">
    <input type="text" style="padding:10px 14px;border-radius:10px;border:none;background:#1a1a2e;color:#fff;font-size:0.95rem;flex:1;min-width:160px" id="urun-filtre" placeholder="Ürün adı filtrele...">
  ...
  <div class="zam-wrap">
    <h3>🔥 En Çok Fiyat Artan Ürünler</h3>
    <div id="zam-listesi"><div class="bos">Yükleniyor...</div></div>
  </div>

  <div class="grafik-wrap" id="grafik-wrap" style="display:none">
...
async function ara() {
  ...
  if (data.length === 0) {
    ...
    tbody.innerHTML = data.map(r=>`<tr onclick="urunGecmis('${r.urun.replace(/'/g,"\\'")}')">
      ...
      <td style="color:#888;font-size:0.82rem">${r.barkod}</td>
    </tr>`).join('');
  }
}
...
async function gecmisYukle() {
  ...
  const urunler = [...new Set(data.map(d=>d.urun))].length;
  const harcama = data.reduce((s,d) => s + parseFloat(d.fiyat||0), 0);
  
  document.getElementById('ozet-kartlar').innerHTML = `
    <div class="card"><div class="sayi">${toplam}</div><div class="etiket">Toplam Kayıt</div></div>
    <div class="card"><div class="sayi">${urunler}</div><div class="etiket">Farklı Ürün</div></div>
    <div class="card"><div class="sayi" style="color:#FF6600">${harcama.toFixed(2)} ₺</div><div class="etiket">Toplam Alış</div></div>
  `;

  // Tablo
  if (data.length === 0) {
    document.getElementById('gecmis-tablo').innerHTML = '<tr><td colspan="4" class="bos">📭 Kayıt bulunamadı. Excel\'e veri ekleyin ve 🔄 yenile butonuna basın.</td></tr>';
  } else {
    document.getElementById('gecmis-tablo').innerHTML = data.map(r=>`
      <tr onclick="tekUrunGrafik('${r.urun.replace(/'/g,"\\'")}')">
        <td style="color:#a8a8b3">${r.tarih}</td>
        <td>${r.urun}</td>
  ...
  if (zamData.length === 0) {
    document.getElementById('zam-listesi').innerHTML = '<div class="bos">Karşılaştırmak için aynı ürünün farklı tarihlerdeki fiyatını girin.</div>';
  } else {
    document.getElementById('zam-listesi').innerHTML = `<table style="width:100%">
      <thead><tr><th>Ürün</th><th>İlk Fiyat</th><th>Son Fiyat</th><th>Değişim</th></tr></thead>
      <tbody>${zamData.map(z => {
        const yuzde = ((z.son - z.ilk) / z.ilk * 100).toFixed(1);
        const renk = z.son > z.ilk ? 'zam-up' : 'zam-down';
        const isaret = z.son > z.ilk ? '▲' : '▼';
        return `<tr><td>${z.urun}</td><td>${z.ilk.toFixed(2)} ₺</td><td>${z.son.toFixed(2)} ₺</td>
          <td><span class="zam-badge ${renk}">${isaret} %${Math.abs(yuzde)}</span></td></tr>`;
      }).join('')}</tbody></table>`;
  }

  // Grafik - filtreli ürün varsa göster
  if (urun && data.length > 0) tekUrunGrafik(urun);
}
...
function tekUrunGrafik(urunAdi) {
  fetch('/gecmis?urun=' + encodeURIComponent(urunAdi)).then(r=>r.json()).then(data => {
    ...
    const etiketler = data.map(d => d.tarih);
    const fiyatlar = data.map(d => parseFloat(d.fiyat));
    ...
    grafik = new Chart(document.getElementById('fiyatGrafik'), {
      ...
      options: {
        ...
        scales: {
          x: { ticks: { color: '#a8a8b3' }, grid: { color: '#ffffff11' } },
...
def gecmis():
    ...
    market_col= next((c for c in cols if 'market' in c.lower()), cols[4] if len(cols)>4 else cols[-1])
    ...
    return jsonify([
        {'tarih': str(r[tarih_col]), 'urun': str(r[urun_col]),
         'fiyat': str(r[fiyat_col]).replace(',','.'), 'market': str(r[market_col])}
